...
def get_command(message:Message)->None:
	...
	bot.send_message(message.from_user.id,
					 'Для начала мне нужно узнать, в каком городе вы ищете отель?(Желательно на латинице)',
...
def get_city(message: Message) -> None:
	...
	if city['sr']:
		...
		bot.send_message(message.from_user.id, 'Попробуйте другой город, также попробуйте написать город на латинице',
						 reply_markup=keyboard_stop)
...
def get_city_correct(message):
	...
	if isinstance(message, CallbackQuery):
		...
		with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
			bot.send_message(message.from_user.id, 'Кажется вы не нажали на кнопку, нажмите на 1 из них',
							 reply_markup=data['keyboard_city'])
	...
	with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
		if (message.text.isdigit() and 0 < int(message.text) < 11) or 'custom_price_high' in data:
			...
			if data['command'] != '/custom' or 'custom_price_high' in data:
				...
				if hotels['data'] is not None and hotels['data']['propertySearch']['properties']:
					...
					for i in range(target):
						...
						if not hotels['data']['propertySearch']['properties'][i]['price']['options']:
							data['hotels_price'].append('Цена не указана')
						elif hotels['data']['propertySearch']['properties'][i]['price']['options'][0]['strikeOut'] is None:
							data['hotels_price'].append('Цена не указана')
						else:
...
def get_hotels_images(message: Message) -> None:
	...
	if message.text.isdigit() and 0 < int(message.text) < 7:
		with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
			...
			some_list = []
			for properid in data['hotels_id']:
				payload = {
					"currency": "USD",
					"eapid": 1,
					"locale": "en_US",
			...
			for num, image_list in enumerate(data['hotels_images']):
				...
				bot.send_message(message.from_user.id, 'Отель: {}\nНаходится по адресу: {}\nЦена за 1 ночь: {}$\n'
													   'Цена за все время проживание: {}$'.format(
														data['hotels_list'][num], data['hotels_addresses'][num],
															  data['hotels_price'][num], tot_price))
